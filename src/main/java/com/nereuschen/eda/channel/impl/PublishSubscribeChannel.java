...
import com.nereuschen.eda.dispatcher.impl.BroadcastingDispatcher;
import com.nereuschen.eda.util.ErrorHandler;
...
public class PublishSubscribeChannel extends AbstractSubscribableChannel {
	...
	/**
	 * Create a PublishSubscribeChannel that will use an {@link Executor} to
	 * invoke the handlers. If this is null, each invocation will occur in the
	 ...
	 * channel. Otherwise, Exceptions will be thrown directly within the sending
	 * Thread. If no ErrorHandler is provided, and this channel does delegate
	 * its dispatching to an Executor, the default strategy is a
	 * {@link MessagePublishingErrorHandler} that sends error messages to the
	 * failed request Message's error channel header if available or to the
	 * default 'errorChannel' otherwise.
	 * 
...
//							new BeanFactoryChannelResolver(
//									this.getBeanFactory()));
//				}
//				this.executor = new ErrorHandlingTaskExecutor(this.executor,
//						this.errorHandler);
//			}
